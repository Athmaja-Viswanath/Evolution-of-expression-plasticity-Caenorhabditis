...
###IX. Expression divergence vs dn/ds - Supplementary
```{r A3_IX_a}
#A For all the genes
...
cor_test_result_cre <- cor.test(plot_data$log2FoldChange, 
                            plot_data$dn.ds_corrected, 
                            method = "pearson")

# Extract r and p-value
correlation_cre_0 <- cor_test_result_cre$estimate  # Pearson's r
p_value_cre <- cor_test_result_cre$p.value       # p-value
...
##Looking at linear model for each type of species-biased genes

library(ggplot2)
library(ggpmisc)

cre_1 = subset(cumulative_enc_deseq2_filt, crexclat == "Higher in Cre")$log2FoldChange
cor_cre <- cor.test(abs(cre_1),subset(cumulative_enc_deseq2_filt, crexclat == "Higher in Cre")$dn.ds_corrected, method = "pearson")

clat_1 = subset(cumulative_enc_deseq2_filt, crexclat == "Higher in Clat")$log2FoldChange
cor_clat = cor.test(abs(clat_1), subset(cumulative_enc_deseq2_filt, crexclat == "Higher in Clat")$dn.ds_corrected)


A3_IX_c = ggplot(plot_data, aes(x = (log2FoldChange), y = dn.ds_corrected, color = crexclat)) +
  geom_point(alpha = 0.6, size = 2.5) +
  geom_smooth(method = "lm", se = FALSE, aes(linetype = crexclat)) +
  stat_poly_eq(
  ...
  scale_color_manual(values = c("Higher in Cre" = "#E41A1C", "Higher in Clat" = "blue")) +
  scale_linetype_manual(values = c("Higher in Cre" = "solid", "Higher in Clat" = "dashed")) +
  
  labs(
    x = expression("log"[2]*" Fold Change"),
    y = expression(d[n]/d[s]),
    title = "Selection vs Expression Divergence",
    color = "Direction",
    linetype = "Direction"
  ) + theme_minimal(base_size = 14)

A3_IX_d = ggplot(plot_data, aes(x = abs(log2FoldChange), y = dn.ds_corrected, color = crexclat)) +
  geom_point(alpha = 0.6, size = 2.5) +
  geom_smooth(method = "lm", se = FALSE, aes(linetype = crexclat)) +
  xlim(0, 15) +
  ylim(0, 1.25)+
...
cor_test_result <- cor.test(plot_data_gs$log2FoldChange, 
                            plot_data_gs$dn.ds_corrected, 
                            method = "pearson")

# Extract r and p-value
correlation_gs_0 <- cor_test_result$estimate  # Pearson's r
p_value_gs <- cor_test_result$p.value       # p-value
...
##Looking at linear model for each type of tissue-biased genes

library(ggplot2)
library(ggpmisc)

gonad_1 = subset(plot_data_gs, gxs == "Gonad-biased")$log2FoldChange
cor_gonad <- cor.test(abs(gonad_1),subset(plot_data_gs, gxs == "Gonad-biased")$dn.ds_corrected)

soma_1 = subset(plot_data_gs, gxs == "Soma-biased")$log2FoldChange
cor_soma = cor.test(abs(soma_1), subset(plot_data_gs, gxs == "Soma-biased")$dn.ds_corrected)


B3_VIII_c = ggplot(plot_data_gs, aes(x = (log2FoldChange), y = dn.ds_corrected, color = gxs)) +
  geom_point(alpha = 0.6, size = 2.5) +
  
  geom_smooth(method = "lm", se = FALSE, aes(linetype = gxs)) +
  ...
  scale_color_manual(values = c("Soma-biased" = "#E41A1C", "Gonad-biased" = "blue")) +
  scale_linetype_manual(values = c("Soma-biased" = "solid", "Gonad-biased" = "dashed")) +
...
B3_VIII_d = ggplot(plot_data_gs, aes(x = abs(log2FoldChange), y = dn.ds_corrected, color = gxs)) +
  geom_point(alpha = 0.6, size = 2.5) +
  xlim(0, 15) +
  ylim(0, 1.25)+
  geom_smooth(method = "lm", se = FALSE, aes(linetype = gxs)) +
...
cor_test_result_mf <- cor.test(plot_data_mf$log2FoldChange, 
                            plot_data_mf$dn.ds_corrected, 
                            method = "pearson")

# Extract r and p-value
correlation_mf_0 <- cor_test_result_mf$estimate  # Pearson's r
p_value_mf <- cor_test_result_mf$p.value       # p-value
...
#Looking at linear model for each type of sex-biased genes

library(ggplot2)
library(ggpmisc)

male_1 = subset(plot_data_mf, mxf == "Male-biased")$log2FoldChange
cor_male <- cor.test(abs(male_1),subset(plot_data_mf, mxf == "Male-biased")$dn.ds_corrected)

female_1 = subset(plot_data_mf, mxf == "Female-biased")$log2FoldChange
cor_female = cor.test(abs(female_1), subset(plot_data_mf, mxf == "Female-biased")$dn.ds_corrected)


C3_VIII_c = ggplot(plot_data_mf, aes(x = (log2FoldChange), y = dn.ds_corrected, color = mxf)) +
  geom_point(alpha = 0.6, size = 2.5) +
  
  geom_smooth(method = "lm", se = FALSE, aes(linetype = mxf)) +
  ...
  scale_color_manual(values = c("Male-biased" = "#E41A1C", "Female-biased" = "blue")) +
  scale_linetype_manual(values = c("Male-biased" = "solid", "Female-biased" = "dashed")) +
...
C3_VIII_d = ggplot(plot_data_mf, aes(x = abs(log2FoldChange), y = dn.ds_corrected, color = mxf)) +
  geom_point(alpha = 0.6, size = 2.5) +
...
##A Orthologous species-biased genes FIGURE 5 DATA
...
#note: It's comparing estimated group-level dN/dS values derived from a Gamma model — which is more appropriate for positive, skewed data like dN/dS — and usually approximates comparisons of geometric means rather than arithmetic means.
```
...
#saving as pdf
ggsave("../Manuscript drafts/Figures/Supplementary/allcat_upset.pdf", width = 34, height = 15)

#Saving as png
ggsave("../Manuscript drafts/Figures/Supplementary/allcat_upset.png", width = 34, height = 15)
